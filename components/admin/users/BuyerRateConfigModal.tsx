...
const BuyerRateConfigModal: React.FC<BuyerRateProps> = ({
  visible,
  onClose,
  onBuyerSet,
}) => {
  ...
  const validateForm = () => {
    ...
    if (
      !buyerRateConfig.milkRate ||
      Number.isNaN(Number(buyerRateConfig.milkRate)) ||
      Number(buyerRateConfig.milkRate) <= 0
    ) {
  ...
  const handleBuyerSave = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        setError(err?.message ?? `Failed to set rate`);
      } finally {
        setIsLoading(false);
        // Reset form
        setBuyerRateConfig({
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity
                style={[styles.createButton, isLoading && styles.loadingButton]}
